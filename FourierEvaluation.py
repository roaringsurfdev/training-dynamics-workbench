...
def get_discrete_fourier_bases(period: int, device:str=None) -> torch.tensor:
    n = period // 2
...
def get_fourier_bases(size, device:str=None) -> torch.tensor:
    # TODO: 
...
def get_basis_coefficients(bases: torch.tensor, weights: torch.tensor) -> torch.tensor:
    coefficients: torch.tensor = bases @ weights
    return coefficients.norm(dim=-1)

def get_dominant_bases_indices(bases: torch.tensor, weights: torch.tensor):
