...
from miscope.analysis.protocols import Analyzer, ActivationContext, CrossEpochAnalyzer
...
class TestInputTraceAnalyzerShapes:
    def test_output_shapes(self):
        ...
        result = InputTraceAnalyzer().analyze(ActivationContext(bundle=_make_bundle(model, probe), probe=probe, analysis_params=context))  # type: ignore[arg-type]
...
class TestSummaryStats:
    ...
    def test_residue_class_accuracy_shape(self):
        ...
        analyzer = InputTraceAnalyzer()
        result = analyzer.analyze(ActivationContext(bundle=_make_bundle(model, probe), probe=probe, analysis_params=context))  # type: ignore[arg-type]
...
class TestViewsRender:
    def test_accuracy_grid_renders(self):
        ...
        epoch_data = InputTraceAnalyzer().analyze(ActivationContext(bundle=_make_bundle(model, probe), probe=probe, analysis_params=context))  # type: ignore[arg-type]
